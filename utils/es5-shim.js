...
(function(root, factory) {
        ...
        keys: function keys(object) {
            ...
            if (!isArgs) {
                for (var name in object) {
                    if (!(skipProto && name === 'prototype') && !(name.slice(0,2) == '__' && name.slice(-2) == '__') && owns(object, name)) {
    ...
    defineProperties(Date.prototype, {
        toISOString: function toISOString() {
            ...
            year = (
            (year < 0 ? '-' : (year > 9999 ? '+' : '')) + strSlice('00000' + Math.abs(year), (0 <= year && year <= 9999) ? -4 : -6));
            ...
            return (
            year + '-' + arraySlice(result, 0, 2).join('-') + 'T' + arraySlice(result, 2).join(':') + '.' + strSlice('000' + originalGetUTCMilliseconds(this), - 3) + 'Z');
        }
        ...
        Date = (function(NativeDate) {
            ...
            var parseShim = function parse(string) {
                ...
                if (match) {
                    ...
                    if (
                    hour < (hasMinutesOrSecondsOrMilliseconds ? 24 : 25) && minute < 60 && second < 60 && millisecond < 1000 && month > -1 && month < 12 && hourOffset < 24 && minuteOffset < 60 // detect invalid offsets
                    &&
                    day > -1 && day < (dayFromMonth(year, month + 1) - dayFromMonth(year, month))) {
                        result = (
                        ((dayFromMonth(year, month) + day) * 24) + hour + (hourOffset * signOffset)) * 60;
                        result = ((
                        ((result + minute + (minuteOffset * signOffset)) * 60) + second) * 1000) + millisecond;
                        if (isLocalTime) {
                            result = toUTC(result);
                        }
